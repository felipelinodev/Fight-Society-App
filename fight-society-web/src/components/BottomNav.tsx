...
import { LayoutDashboard, Users, Swords, CreditCard, QrCode, User as UserIcon, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-[92%] max-w-sm">
      <div className="p-2 rounded-full bg-slate-950/90 backdrop-blur-xl border border-white/10 shadow-2xl flex items-center justify-between px-3">
        {/* Tab 1: Home (Meu Passe para Aluno, Visão Geral para Admin) */}
        <button
          onClick={() => onSelectTab('home')}
          className={`flex flex-col items-center gap-0.5 p-2.5 rounded-full transition-all ${
            currentTab === 'home'
        ...
        >
          <LayoutDashboard size={19} className={currentTab === 'home' ? 'stroke-[2.5]' : 'stroke-2'} />
        </button>
        ...
        {isAdmin ? (
          ...
          >
            <Users size={19} className={currentTab === 'students' ? 'stroke-[2.5]' : 'stroke-2'} />
          </button>
        ) : (
          <button
            ...
            title="Planos & Matrícula"
          >
            <Swords size={19} className={currentTab === 'plans' ? 'stroke-[2.5]' : 'stroke-2'} />
          </button>
        )}
        ...
        {/* Tab 4: Admin vê "Planos", Aluno vê "Meus Pagamentos" */}
        {isAdmin ? (
          <button
            onClick={() => onSelectTab('plans')}
            className={`flex flex-col items-center gap-0.5 p-2.5 rounded-full transition-all ${
          ...
          >
            <CreditCard size={19} className={currentTab === 'payments' ? 'stroke-[2.5]' : 'stroke-2'} />
          </button>
        )}

        {/* Tab 5: Admin vê "Financeiro", Aluno vê "Meu Perfil" */}
        {isAdmin ? (
          <button
            onClick={() => onSelectTab('payments')}
            className={`flex flex-col items-center gap-0.5 p-2.5 rounded-full transition-all ${
          ...
          <button
            ...
            title="Meu Perfil"
          >
            <UserIcon size={19} className={currentTab === 'profile' ? 'stroke-[2.5]' : 'stroke-2'} />
          </button>
        )}
      </div>
    </div>
